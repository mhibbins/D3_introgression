...
#Function for Peter's version of D3

def calculate_D3_v2(sp2_genome, sp3_genome, sp4_genome):
	
	D23, D24, D34 = 0, 0, 0

	for i in range(len(sp2_genome)):
		if sp2_genome[i] != sp3_genome[i]:
			D23 += 1
		if sp2_genome[i] != sp4_genome[i]:
			D24 += 1
	...
	D23, D24, D34 = float(D23)/len(sp1_genome), float(D24)/len(sp1_genome), float(D34)/len(sp1_genome)
...
def D_D3_block_bootstrap(sp1_genome, sp2_genome, sp3_genome, sp4_genome, n_replicates):
	...
	for i in range(n_replicates): #for each bootstrap replicate
		...
		D3_v2_estimates.append(calculate_D3_v2(sp2_bootstrapped, sp3_bootstrapped, sp4_bootstrapped))
	...
	return D_stdev, D3_stdev, D3_v2_stdev

print(calculate_D(sp1_genome, sp2_genome, sp3_genome, sp4_genome), calculate_D3(sp2_genome, sp3_genome, sp4_genome), calculate_D3_v2(sp2_genome, sp3_genome, sp4_genome),  D_D3_block_bootstrap(sp1_genome, sp2_genome, sp3_genome, sp4_genome, 1000))
