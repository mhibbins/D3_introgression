...
def D_D3_block_bootstrap(sp1_genome, sp2_genome, sp3_genome, sp4_genome, n_replicates):
	...
	for i in range(n_replicates): #for each bootstrap replicate
		...
		for j in range(100): #for each resampling window
			...
			sampling_index = random.randint(0, 99000) #index to slice 

			#sample windows 
			sp1_bootstrapped.append(sp1_genome[sampling_index:sampling_index+1000])
			sp2_bootstrapped.append(sp2_genome[sampling_index:sampling_index+1000])
			sp3_bootstrapped.append(sp3_genome[sampling_index:sampling_index+1000])
			sp4_bootstrapped.append(sp4_genome[sampling_index:sampling_index+1000])
